...
def NaiveMonteCarlo(option, spot, rf, dividend, volatility, T, steps, reps):
    H = T/steps
    z = np.random.normal(size=reps)
    spot_t = spot * np.exp((rf - dividend - 0.5 * volatility * volatility) * H + volatility * np.sqrt(H)*z)
    payoff_t = option.payoff(spot_t)
    price = payoff_t.mean() * np.exp(-rf * H)
    return price
...
def main():
    ...
    x = EuropeanBinomialModel(option,S,rf,dividend,volatility,T,N)
    y = AmericanBinomialModel(option,S,rf,dividend,volatility,T,N)
    m = NaiveMonteCarlo(option,S,rf,dividend,volatility,T,steps,reps)
    ...
    print("The Naive Monte Carlo Price is = {0:.4f}".format(m))
